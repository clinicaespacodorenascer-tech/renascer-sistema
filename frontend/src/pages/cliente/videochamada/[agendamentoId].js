...
export default function VideoChamada() {
  ...
  async function iniciar() {
    const { data } = await api.post(`/cliente/agenda/${agendamentoId}/iniciar-chamada`);
    setChamada(data);
    setAviso(data.aviso || "");
    if (data.linkMeet) {
  ...
  async function encerrar() {
    const { data } = await api.post(`/cliente/agenda/${agendamentoId}/encerrar-chamada`);
    setChamada(data);
  }
  ...
  return (
    <Layout user={user}>
      <div className="card max-w-2xl mx-auto text-center">
        <h1 className="text-xl font-semibold mb-3">Sua sessão por videochamada</h1>
